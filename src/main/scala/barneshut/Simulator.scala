...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries = {
    ...
    br.minX = Math.min(boundaries.minX, body.x)
    br.maxX = Math.max(boundaries.maxX, body.x)
    br.minY = Math.min(boundaries.minY, body.y)
    br.maxY = Math.max(boundaries.maxY, body.y)
    br
  }

  def mergeBoundaries(a: Boundaries, b: Boundaries): Boundaries = {
    val br = new Boundaries()
    br.minX = Math.min(a.minX, b.minX)
    br.maxX = Math.max(a.maxX, b.maxX)
    br.minY = Math.min(a.minY, b.minY)
    br.maxY = Math.max(a.maxY, b.maxY)
    br
  }
